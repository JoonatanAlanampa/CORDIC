...
uo[0] and quadrature cosine on uo[1]. An external RC low-pass (1k +
...
uo[7:3] show the live sine level as an offset-binary bar — at low
...
filter on uo[0], and watch a sine on a scope (or listen to it — audio
range). The uo[7:3] level bar breathes at the output frequency.

## External hardware

None required (LED bar works bare). For analog output: an RC low-pass on
uo[0]/uo[1] — or the Tiny Tapeout audio Pmod. Any SPI master (demo board
